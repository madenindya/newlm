...
echo "Run Finetuning Grid Search for BERT $task with GPU $gpu"
...
do
    ...
    do
        ...
        do
            ...
            --config_file="examples/configs_gcloud/run-ft.bert.yaml" \
            --bs=$bs --lr=$lr --seed=$seed --tasks=[$task]
        done
    done
done

python summarize_tuning.py outputs/en.100-percent.bert-finetune $task
